    ...
    public class PreviewService : IPreviewService
    {
        public async Task<PreviewDataDto> GetPreviewDataAsync(string tmpdir, int mappingStep, string selectedColumn)
        {
            ...
            try
            {
                ...
                await LoadImportColumnsAsync(connection, importTable!, model).ConfigureAwait(false);
                await LoadClaimsColumnsAsync(connection, model).ConfigureAwait(false);
                await LoadPreviewRowsAsync(connection, importTable!, model).ConfigureAwait(false);
        ...
        private static async Task TryLoadSavedMappingsAsync(string tmpdir, PreviewDataDto model)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (root.TryGetProperty("translationMapping", out System.Text.Json.JsonElement arr) && arr.ValueKind == System.Text.Json.JsonValueKind.Array)
                    {
                        foreach (System.Text.Json.JsonElement el in arr.EnumerateArray())
                        {
                            ...
                            var input = el.TryGetProperty("inputColumn", out System.Text.Json.JsonElement i) ? i.GetString() : null;
                            var output = el.TryGetProperty("outputColumn", out System.Text.Json.JsonElement o) ? o.GetString() : null;
