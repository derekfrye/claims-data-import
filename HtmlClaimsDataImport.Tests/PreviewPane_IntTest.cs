...
[Collection("WebApp")]
public class PreviewPaneIntegrationTests(WebApplicationFactory<Program> factory)
{
    ...
    private static async Task<string> LoadDataAsync(HttpClient client, string tmpdir, string csvFileName, string jsonPath = "default", string databasePath = "default")
    {
        var token = await GetAntiForgeryTokenAsync(client);
    ...
    public async Task PreviewPane_LoadData_ShowsTableWithData()
    {
        ...
        Assert.Contains(firstRowCells, cell => cell.TextContent.Contains("1")); // id
        Assert.Contains(firstRowCells, cell => cell.TextContent.Contains("100.5")); // amount (no trailing zero)
    }
    ...
    public async Task PreviewPane_ClickColumn_HighlightsEntireColumn()
    {
        ...
        var csvContent = "ClaimID,Amount,Date,Description\n1,100.50,2024-01-15,Medical claim\n2,250.75,2024-01-16,Dental claim";
        var (_, tmpdir) = await UploadTestFileAsync(sessionClient, "filename", "test-claims.csv", csvContent);
        await LoadDataAsync(sessionClient, tmpdir, "test-claims.csv");
        ...
        Assert.Contains(selectedCells, cell => cell.TextContent.Contains("100.5"));
    ...
    public async Task PreviewPane_ColumnSelection_UpdatesMappingStep()
    {
        ...
        // Setup test data
        var csvContent = "RefID,Price,Created,Notes\n1,99.99,2024-01-01,Test note";
        var (_, tmpdir) = await UploadTestFileAsync(sessionClient, "filename", "test-data.csv", csvContent);
        await LoadDataAsync(sessionClient, tmpdir, "test-data.csv");
    ...
    public async Task PreviewPane_CornflowerBlueHighlighting_CorrectCssApplied()
    {
        ...
        var csvContent = "ItemName,Value,Status\nTest1,123,Active\nTest2,456,Inactive";
        var (_, tmpdir) = await UploadTestFileAsync(sessionClient, "filename", "test.csv", csvContent);
        await LoadDataAsync(sessionClient, tmpdir, "test.csv");

        // Get preview with "Value" column selected
        var previewHtml = await GetPreviewAsync(sessionClient, tmpdir, 0, "value");
    ...
    public async Task PreviewPane_ClickableCells_HaveCorrectOnclickAttributes()
    {
        ...
        var (_, tmpdir) = await UploadTestFileAsync(sessionClient, "filename", "clickable-test.csv", csvContent);
        await LoadDataAsync(sessionClient, tmpdir, "clickable-test.csv");
    ...
    public async Task PreviewPane_NavigationButtons_WorkCorrectly()
    {
        ...
        var (_, tmpdir) = await UploadTestFileAsync(sessionClient, "filename", "nav-test.csv", csvContent);
        await LoadDataAsync(sessionClient, tmpdir, "nav-test.csv");
    ...
    public async Task PreviewPane_NoColumnSelected_NextButtonDisabled()
    {
        ...
        var (_, tmpdir) = await UploadTestFileAsync(sessionClient, "filename", "button-test.csv", csvContent);
        await LoadDataAsync(sessionClient, tmpdir, "button-test.csv");
    ...
    public async Task PreviewPane_SelectDifferentColumns_HighlightsCorrectColumn(string columnToSelect)
    {
        ...
        var (_, tmpdir) = await UploadTestFileAsync(sessionClient, "filename", "multi-column-test.csv", csvContent);
        await LoadDataAsync(sessionClient, tmpdir, "multi-column-test.csv");
