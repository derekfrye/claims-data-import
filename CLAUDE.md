...
### Automated Build and Analysis Script
```bash
# Comprehensive build and analyzer diagnostic summary
./build_and_analyze_style.sh

# Build with warnings as errors (recommended)
./build_and_analyze_style.sh -w

# Build specific project with per-project rule breakdown
./build_and_analyze_style.sh -w -s HtmlClaimsDataImport/HtmlClaimsDataImport.csproj -p HtmlClaimsDataImport.csproj

# Build library only
./build_and_analyze_style.sh -w -s LibClaimsDataImport/LibClaimsDataImport.csproj
```

This script provides:
- Clean and build with timestamped logs in `build_artifacts/`
- Analyzer diagnostic summaries (top rules by frequency, rules by project)
- Convenient symlinks to latest logs: `build_artifacts/clean.log`, `build_artifacts/build.log`
- Diagnostic analysis files: `warnings_by_id.txt`, `warnings_by_project.txt`, `diagnostics_samples.txt`
...
### Meziantou.Analyzer
```bash
# Meziantou.Analyzer (v2.0.212) is configured in:
# - LibClaimsDataImport project
# - CmdClaimsDataImport project  
# - HtmlClaimsDataImport project
# Provides additional code quality rules and performance optimizations
# Integrated with build process - warnings appear during dotnet build
```
...
- Meziantou.Analyzer provides additional performance and code quality rules
- Tests must pass before committing changes
- Use `./build_and_analyze_style.sh -w` for comprehensive build and diagnostic analysis

### Build Artifacts System
The `build_artifacts/` directory contains structured build analysis:
- **Timestamped logs**: `clean_YYYYMMDD_HHMMSS.log`, `build_YYYYMMDD_HHMMSS.log`
- **Latest symlinks**: `clean.log`, `build.log` for easy access
- **Diagnostic summaries**: 
  - `warnings_by_id.txt` - Top analyzer rules by frequency
  - `warnings_by_project.txt` - Projects with most diagnostics
  - `diagnostics_samples.txt` - Sample diagnostic messages
- Generated by `build_and_analyze_style.sh` for build quality tracking
...
- **AGENTS.md**: Repository-specific guidelines for coding style, naming conventions, build commands, testing, and commit practices. Complements this file with detailed developer workflow information.